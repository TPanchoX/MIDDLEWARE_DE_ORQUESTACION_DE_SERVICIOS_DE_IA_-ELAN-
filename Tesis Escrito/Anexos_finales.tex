...
\renewcommand{\theHsubsubsection}{A.\arabic{subsection}.\arabic{subsubsection}}
\setcounter{subsection}{0}
\setcounter{subsubsection}{0}

\subsection{Esquema completo del archivo \textbf{manifest.json}}
...
El documento JSON se organiza en campos de primer nivel que describen la identidad del modelo, su forma de ejecución, sus artefactos y los valores que verá el usuario en ELAN:

\begin{enumerate}[noitemsep, topsep=0pt, parsep=0pt, partopsep=0pt]
    \item \textbf{model\_id}: identificador único del modelo dentro del registro.
    \item \textbf{name}: nombre legible por el usuario, mostrado en la interfaz de ELAN.
    \item \textbf{version}: versión semántica del modelo, por ejemplo, \textbf{1.0.0}.
    \item \textbf{task}: tipo de tarea que realiza el modelo.
    \item \textbf{runtime}: modo de ejecución y tipo de runner.
    \item \textbf{artifacts}: mapa de rutas relativas a los artefactos del paquete.
    \item \textbf{input\_contract} y \textbf{output\_contract}: describen el medio de entrada y el tipo de salida que produce el modelo.
    \item \textbf{container}: configuración de red del contenedor Docker. Es obligatorio cuando se utiliza \textbf{runner: docker\_http}.
    \item \textbf{ui}: valores predeterminados para el panel de configuración de ELAN.
\end{enumerate}

\subsubsection{Definición de campos}

\begin{table}[H]
\centering
\caption{Campos del objeto raíz del \textbf{manifest.json}}
...
\subsubsection{Valores admitidos para \textbf{task}}

El esquema \textbf{ModelManifest} del middleware acepta exactamente dos valores para este campo; cualquier otro provoca el rechazo del paquete durante la validación:

\begin{itemize}[noitemsep, topsep=0pt, parsep=0pt, partopsep=0pt]
    \item \textbf{video\_segmentation}: el modelo detecta regiones temporales en el video sin clasificar su contenido semántico.
    \item \textbf{video\_segmentation\_and\_gloss\_classification}: el modelo detecta regiones temporales y asigna una glosa de lengua de señas a cada segmento. Este valor corresponde al modelo de referencia.
\end{itemize}

\subsubsection{Objeto \textbf{runtime}}

\begin{table}[H]
\centering
\caption{Campos del objeto \textbf{runtime}}
...
\subsubsection{Objeto \textbf{artifacts}}

El middleware no interpreta el contenido de estos artefactos; su función es conservar las rutas declaradas para que el \textbf{Dockerfile} del paquete pueda construir la imagen del backend con los archivos correctos. Las claves del mapa actúan como nombres semánticos definidos por el autor del modelo, mientras que los valores apuntan a rutas relativas dentro del paquete. Durante la instalación se comprueba que cada archivo declarado exista dentro del ZIP y que ninguna ruta sea absoluta ni contenga \textbf{../}; si alguna condición falla, el paquete se rechaza antes de construir la imagen.

\begin{table}[H]
\centering
\caption{Artefactos del modelo de referencia \textbf{lsec\_bio\_gloss\_final\_v1}}
...
\subsubsection{Objeto \textbf{container}}

Este objeto es obligatorio cuando \textbf{runtime.runner = docker\_http}, porque reúne los datos que el \textbf{DockerRunner} necesita para iniciar el backend, comprobar su disponibilidad y enviarle las solicitudes de inferencia.

\begin{table}[H]
\centering
\caption{Campos del objeto \textbf{container}}
...
\subsubsection{Objeto \textbf{ui}}

Los valores de este objeto se cargan automáticamente en el panel \textbf{AIOrchestrationRecognizerPanel} cuando el usuario selecciona un modelo en ELAN. Con ello se evita repetir configuraciones básicas en cada ejecución y se mantiene una experiencia más cercana al flujo habitual de anotación.

\begin{table}[H]
\centering
\caption{Campos del objeto \textbf{ui}}
...
\subsubsection{Ejemplo completo: \textbf{lsec\_bio\_gloss\_final\_v1}}

El siguiente listado presenta el \textbf{manifest.json} completo del modelo de referencia utilizado en la validación del sistema. Al incluirse en la raíz del paquete ZIP, este archivo se convierte en la fuente de información que guía la instalación, el registro del modelo y la configuración inicial mostrada en ELAN. Nótese la diferencia entre los dos timeouts de arranque: \textbf{backend\_config} usa 180 segundos porque durante la instalación el backend carga los pesos por primera vez, mientras que \textbf{container} usa 30 segundos porque en cada inferencia el contenedor ya debería estar preparado.
...
\captionof{figure}{Manifest del modelo \textbf{lsec\_bio\_gloss\_final\_v1} v1.0.0}
\label{fig:manifest_ejemplo}
\end{minipage}
\end{center}

% ============================================================
%  Anexo B — Ejemplo completo de respuesta JSON de inferencia
% ============================================================
...
\renewcommand{\theHsubsection}{B.\arabic{subsection}}
\renewcommand{\theHsubsubsection}{B.\arabic{subsection}.\arabic{subsubsection}}
\setcounter{subsection}{0}
\setcounter{subsubsection}{0}

\subsection{Ejemplo completo de respuesta JSON de inferencia}
...
\captionof{figure}{Request de inferencia enviado al endpoint \textbf{POST /api/v1/jobs/segment-video}}
\label{fig:request_inferencia}
\end{minipage}
\end{center}

\subsubsection{Respuesta completa HTTP 200}

La respuesta incluye cinco campos de primer nivel: \textbf{job\_id}, \textbf{status}, \textbf{media\_info}, \textbf{segments} y \textbf{trace}. Los cuatro primeros identifican el trabajo, su estado, el video procesado y los segmentos obtenidos. El campo \textbf{trace}, por su parte, agrega el historial de estados, los tiempos de cada etapa del pipeline y los metadatos necesarios para observar la ejecución del contenedor Docker. Por brevedad, el listado muestra solo el primero de los cuatro segmentos detectados (\textbf{n\_detected\_segments}); los restantes siguen exactamente la misma estructura.
...
\captionof{figure}{Respuesta completa de inferencia exitosa HTTP 200}
\label{fig:respuesta_inferencia}
\end{minipage}
\end{center}

\subsubsection{Descripción de los campos del objeto \textbf{trace}}

\begin{table}[H]
\centering
\caption{Campos del objeto \textbf{trace} en la respuesta de inferencia}
...
\subsubsection{Desglose del objeto \textbf{stages}}

El objeto \textbf{stages} corresponde al esquema \textbf{StageMetrics} definido en \textbf{middleware/app/schemas/metrics.py}. Su utilidad principal es separar la latencia total en dos niveles: las etapas propias del orquestador y las etapas internas del pipeline de IA. Esta separación facilita identificar si el tiempo de respuesta se consume en validación, espera, comunicación con Docker o procesamiento del modelo.

\begin{table}[H]
\centering
\caption{Significado de los campos del objeto \textbf{stages}}
...
% ============================================================
%  Anexo C — Configuración completa del docker-compose.yml
% ============================================================
...
\renewcommand{\thesubsubsection}{C.\arabic{subsection}.\arabic{subsubsection}}
\renewcommand{\theHsubsection}{C.\arabic{subsection}}
\renewcommand{\theHsubsubsection}{C.\arabic{subsection}.\arabic{subsubsection}}
\setcounter{subsection}{0}
\setcounter{subsubsection}{0}

\subsection{Configuración completa del \textbf{docker-compose.yml} del middleware}
\label{anx:docker_compose}

El archivo \textbf{docker-compose.yml} reúne la configuración necesaria para ejecutar el middleware como servicio local y conectarlo con los contenedores de modelos de IA. Es el mecanismo recomendado para el despliegue, porque concentra en un solo lugar el ciclo de vida del contenedor, las variables de entorno, los bind mounts de persistencia y la red compartida \textbf{elan-ai-shared}.

\subsubsection{Archivo \textbf{docker-compose.yml} completo}
...
\captionof{figure}{Archivo \textbf{docker-compose.yml} del middleware}
...
Las variables que no aparecen declaradas en el archivo, como \textbf{MIDDLEWARE\_MAX\_CONCURRENT\_JOBS}, toman su valor por defecto desde \textbf{app/core/config.py}. Para modificarlas basta con añadirlas a la sección \textbf{environment} del servicio y reiniciar el contenedor.

\subsubsection{Bind mounts de persistencia}

La configuración utiliza únicamente bind mounts, es decir, directorios del equipo anfitrión montados dentro del contenedor. Esta decisión evita depender de volúmenes Docker con nombre y facilita revisar los archivos directamente desde el sistema operativo. Además, los datos de modelos y manifests se conservan incluso después de detener el servicio o limpiar volúmenes desde Docker.
...
\subsubsection{Red Docker \textbf{elan-ai-shared}}

La red \textbf{elan-ai-shared} conserva un nombre fijo, independiente del prefijo que Docker Compose asigna al proyecto. Esto permite que los contenedores de modelo, creados dinámicamente por el SDK de Docker durante la inferencia, se unan a la misma red sin configuraciones adicionales. Una vez conectados, el middleware puede comunicarse con cada backend por su nombre de contenedor mediante la resolución DNS interna de Docker, sin exponer puertos innecesarios en el host.

\subsubsection{Comandos de operación}
...
\captionof{figure}{Comandos Docker Compose más frecuentes durante la operación}
\label{fig:compose_cmds}
\end{minipage}
\end{center}

\subsubsection{Consideraciones de seguridad y red}

El puerto 8000 del middleware se expone únicamente en la interfaz de loopback del host, mediante \textbf{127.0.0.1:8000}. Con esta decisión, el servicio queda disponible para ELAN en la misma máquina, pero no para otros equipos de la red local. Esta restricción encaja con el escenario previsto del TIC, en el que el investigador ejecuta ELAN, el middleware y Docker en su propio equipo de trabajo.
...
% ============================================================
%  Anexo D — Instrucciones de compilación e integración en ELAN 7.1
% ============================================================
\section*{ANEXO D}
\addcontentsline{toc}{section}{ANEXO D}

\renewcommand{\thesubsection}{D.\arabic{subsection}}
\renewcommand{\thesubsubsection}{D.\arabic{subsection}.\arabic{subsubsection}}
\renewcommand{\theHsubsection}{D.\arabic{subsection}}
\renewcommand{\theHsubsubsection}{D.\arabic{subsection}.\arabic{subsubsection}}
\setcounter{subsection}{0}
\setcounter{subsubsection}{0}

\subsection{Instrucciones de compilación e integración con ELAN}
...
Antes de ejecutar Maven, la variable de entorno \textbf{JAVA\_HOME} debe apuntar al JDK 21. Esta verificación evita errores de compilación asociados a versiones de Java distintas de las declaradas en el proyecto.

\subsubsection{Estructura del código añadido}

Las clases Java añadidas se concentran en un solo paquete dentro del repositorio de ELAN. La siguiente estructura muestra la ubicación del reconocedor, su panel de configuración, el cliente HTTP y los DTO utilizados para intercambiar datos con el middleware:
...
\subsubsection{Dependencias añadidas al \textbf{pom.xml}}

El cliente HTTP \textbf{AIOrchestrationMiddlewareClient} usa clases disponibles en el JDK 11+, en particular \textbf{java.net.http.HttpClient}, por lo que las llamadas al middleware no requieren librerías externas. Para el parseo de las respuestas JSON se utiliza \textbf{org.json}, dependencia que ya forma parte del archivo \textbf{pom.xml} de ELAN 7.1. En consecuencia, la integración no agrega nuevas dependencias al proyecto.

\subsubsection{Registro del reconocedor en ELAN}

ELAN descubre los reconocedores disponibles mediante el mecanismo Service Provider Interface de Java. Por ello, el nuevo reconocedor debe declararse en el archivo \textbf{src/main/resources/META-INF/services/mpi.eudico.client.annotator.recognizer.api.Recognizer} con la siguiente línea:
...
\captionof{figure}{Línea añadida al archivo SPI para registrar el reconocedor de IA}
\label{fig:spi_recognizer}
\end{minipage}
\end{center}

La línea se añade al final del archivo existente, sin eliminar otras implementaciones que ya estén registradas. Al iniciar la función \textit{Run Recognizer}, ELAN lee este archivo y carga todos los reconocedores declarados.

\subsubsection{Compilación con Maven}

La compilación sigue el ciclo de vida estándar de Maven. Los comandos deben ejecutarse desde el directorio raíz del proyecto \textbf{elan-7.1/}, de modo que Maven encuentre el archivo \textbf{pom.xml}, resuelva las dependencias y genere el JAR final:
...
\captionof{figure}{Proceso de compilación y empaquetado con Maven}
\label{fig:maven_build}
\end{minipage}
\end{center}

Si la compilación termina correctamente, Maven reporta \textbf{BUILD SUCCESS} y deja el archivo JAR generado en \textbf{target/elan-7.1.jar}.

\subsubsection{Ejecución directa con Maven en modo desarrollo}

Durante el desarrollo, ELAN puede ejecutarse directamente desde Maven. Esta opción permite probar el reconocedor integrado sin reemplazar todavía el JAR de una instalación existente:
...
\captionof{figure}{Ejecución de ELAN durante el desarrollo sin construir el JAR}
\label{fig:maven_run}
\end{minipage}
\end{center}

\subsubsection{Distribución del JAR compilado}

Para distribuir ELAN con el reconocedor de IA integrado, se puede reemplazar el JAR original de una instalación existente. Antes de hacerlo, conviene guardar una copia de seguridad para poder restaurar la herramienta si el nuevo paquete no inicia correctamente:
...
\captionof{figure}{Sustitución del JAR en una instalación existente de ELAN en Windows}
\label{fig:reemplazar_jar}
\end{minipage}
\end{center}

\noindent\textbf{Nota:} si la instalación de ELAN utiliza un \textit{launcher} independiente, como ocurre con algunos instaladores de MPI, el nombre del JAR puede variar. Por esta razón, antes de reemplazarlo se debe revisar qué archivo ejecuta realmente el script de lanzamiento o el acceso directo de ELAN.

\subsubsection{Verificación de la integración}
