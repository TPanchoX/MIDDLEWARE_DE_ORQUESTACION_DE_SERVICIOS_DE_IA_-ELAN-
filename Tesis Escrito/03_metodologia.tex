...
Este capítulo describe la metodología para desarrollar y validar el middleware de orquestación integrado con ELAN. La metodología se organiza en diez apartados: enfoque y tipo de trabajo, técnicas de recolección de información, técnicas de análisis de la información, herramientas y stack tecnológico, proceso de desarrollo iterativo, contratos de comunicación, despliegue y configuración del entorno, estructura de directorios del sistema, guía de replicación y uso ético de herramientas de inteligencia artificial generativa. Para evitar ambigüedades, en este capítulo se utiliza el término \textit{apartado} para referirse a la estructura metodológica general y el término \textit{fase} solo para los incrementos del proceso de desarrollo.

El apartado de desarrollo iterativo se compone de una etapa de análisis y diseño, seguida de seis fases de implementación: base del middleware, registro de modelos e instalación automática, Docker Runner e inferencia, cola de jobs y métricas, integración con la interfaz de ELAN y validación final. Esta organización permite diferenciar la estructura expositiva del capítulo respecto de los incrementos técnicos ejecutados durante el desarrollo.

El objetivo de esta metodología no es solo enumerar actividades, sino también explicar cómo el problema de integración se convirtió en una arquitectura coherente. El sistema desarrollado conecta una aplicación de escritorio en Java, un middleware en Python, servicios HTTP, contenedores Docker y backends de inferencia; por ello, cada decisión metodológica se justifica según su aporte a la interoperabilidad, la separación de responsabilidades, la validación de contratos y la reducción de dependencias entre componentes.

\subsection{Enfoque y tipo de trabajo}

El trabajo es de naturaleza aplicada y tecnológica. No se trata de proponer nuevas ideas en aprendizaje automático, ni de aportar un análisis lingüístico de la LSEc, sino de diseñar, implementar y validar un componente de software que resuelve un problema concreto de integración: lograr que ELAN solicite inferencias a servicios de IA externos y reciba segmentos temporales compatibles con su flujo de anotación, sin alterar la arquitectura general de la herramienta y sin obligar al investigador a transcribir manualmente los resultados.

El componente se desarrolló de manera iterativa e incremental, tomando de los marcos ágiles la idea de ciclos cortos con revisión y adaptación continuas. El sistema creció en incrementos verificables: primero la API base, después el registro de modelos, la ejecución de contenedores, la cola de trabajos, la integración con ELAN y, al final, la validación de escenarios de fallo. Este enfoque resultó adecuado porque al inicio no se conocían todas las restricciones de integración entre ELAN, AVATecH, el cliente Java y el servidor del middleware; cada iteración sirvió para confirmar una decisión técnica, detectar incompatibilidades y ajustar componentes ya implementados sin rediseñar el sistema completo.
...
\subsection{Proceso de desarrollo: fases iterativas}

El desarrollo se estructuró en una etapa inicial de análisis y diseño seguida de seis fases de implementación, lo que permitió avanzar desde la definición de la arquitectura hasta la validación del sistema completo sin perder de vista el flujo real de trabajo en ELAN. Cada fase dejó un producto operativo: la API base, la instalación de modelos, el ejecutor Docker, la cola de trabajos con métricas, la integración gráfica y las pruebas de extremo a extremo.
...
Se analizaron tres alternativas de integración. La primera fue ejecutar procesos externos desde ELAN mediante línea de comandos o archivos intermedios. Aunque mantenía el modelo fuera de ELAN, complicaba el reporte de progreso, la cancelación y la conversión de resultados a estructuras compatibles con el anotador. La segunda fue integrar el backend de IA dentro del proceso de ELAN mediante JNI, pero esta opción introducía una frontera compleja entre Java, Python, bibliotecas nativas y dependencias del modelo \cite{oracle_jni}. Además, reducía la portabilidad, porque el usuario tendría que mantener un entorno compatible con esas dependencias.

La alternativa adoptada fue implementar en ELAN un reconocedor Java que actúe como cliente HTTP de un servicio local. Con esta decisión, ELAN conserva su flujo de configuración e inserción de anotaciones, mientras que el middleware coordina los backends de IA fuera del proceso principal. La comunicación HTTP/1.1 con JSON se eligió por ser explícita, verificable y desacoplada, de acuerdo con el estilo REST \cite{fielding2000rest}. Para el cliente Java se usó \textbf{java.net.http.HttpClient}, disponible desde Java 11, evitando añadir librerías externas al código de ELAN \cite{java11_httpclient}.
...
\captionof{listing}{Formato uniforme de respuesta de error del middleware}
...
\captionof{listing}{Respuesta esperada del endpoint de salud del middleware}
...
\captionof{listing}{Estructura esperada del paquete ZIP de un modelo}
\label{lst:estructura_zip_modelo}
\end{minipage}
\end{center}

Para mejorar la resiliencia se implementaron \textit{bootstrap manifests}. Durante la instalación, el middleware guarda una copia del manifest en \textbf{data/bootstrap\_manifests/} con el nombre \textbf{<model\_id>\_\_<version>.json}. Al iniciar, \textbf{\_bootstrap\_docker\_manifests()} escanea ese directorio y re-registra modelos si \textbf{registry.json} fue eliminado, siempre que el bind mount persistente siga disponible.
...
\captionof{listing}{Patrón de nombre del contenedor Docker de un modelo}
...
El primer componente fue \textbf{JobQueue}. Se descartó el uso de un semáforo porque limitaría la concurrencia sin garantizar el orden FIFO; en su lugar se combinó una cola \textbf{collections.deque}, un \textbf{threading.Event} por cada job en espera y un \textbf{threading.Lock} que protege el contador de slots activos. Cuando no hay slots libres, el job se añade al final del deque y su hilo queda bloqueado; al terminar un job, el slot se libera y se despierta al job más antiguo en espera. Así se controla la concurrencia y se garantiza un trato equitativo entre solicitudes \cite{python_threading}.
...
El segundo componente fue \textbf{MetricsService}. Este servicio mantiene contadores protegidos por \textbf{threading.Lock}: jobs recibidos, completados, fallidos, expirados por timeout y un conteo de errores por código. Los campos \textbf{active\_jobs} y \textbf{queued\_jobs} no son contadores, sino un snapshot del estado de \textbf{JobQueue} tomado al consultar \textbf{GET /api/v1/metrics}.

La integración con Uvicorn exigió considerar que \textbf{POST /api/v1/jobs/segment-video} se ejecuta como manejador sincrónico: si no hay slots libres, el hilo queda bloqueado en \textbf{JobQueue.submit()} hasta que llegue su turno. La configuración del servidor debe tolerar solicitudes en espera y, al mismo tiempo, seguir atendiendo endpoints de gestión como \textbf{GET /health} o las métricas; si se amplía la concurrencia, el número de workers de Uvicorn debe crecer en proporción \cite{uvicorn_docs}.
...
La implementación en \textbf{job\_service.py} captura \textbf{DockerTimeoutError} antes que el manejador genérico de \textbf{DockerServiceError}. Cuando ocurre, el job termina en \textbf{TIMEOUT}, \textbf{MetricsService} incrementa \textbf{timeout\_jobs} y el middleware responde HTTP 504 con \textbf{DOCKER\_TIMEOUT}. Esta distinción se usó en las pruebas de carga y en el escenario de timeout forzado.
...
El método \textbf{installModel} envía el ZIP como \textbf{multipart/form-data}. Java 11 no ofrece un constructor nativo de cuerpos multipart en \textbf{HttpClient}, por lo que el cuerpo se arma manualmente: boundary, cabeceras de la parte \textbf{file}, bytes del ZIP y cierre del multipart. La estructura enviada conserva el formato esperado por FastAPI para recibir archivos:
...
\captionof{listing}{Cuerpo multipart enviado por el cliente Java para instalar modelos}
\label{lst:multipart_install_model}
\end{minipage}
\end{center}

Durante las pruebas se observó que el header \textbf{Content-Type} no debía incluir los dos guiones iniciales del boundary, aunque los delimitadores del cuerpo sí los requieren. Por ello se declaró un boundary como \textbf{FormBoundary<uuid>} en el header y se conservaron los guiones en el cuerpo, conforme al formato \textbf{multipart/form-data} \cite{rfc7578}.
...
\captionof{listing}{Solicitud JSON enviada desde ELAN al middleware}
...
\captionof{listing}{Payload interno enviado desde el middleware al backend}
...
\captionof{listing}{Comandos iniciales para desplegar y verificar el middleware}
\label{lst:comandos_despliegue_middleware}
\end{minipage}
\end{center}

La integración con ELAN requiere compilar el árbol fuente modificado con Maven e incorporar el JAR resultante a la instalación de la herramienta; el procedimiento paso a paso, incluido el registro del reconocedor mediante el mecanismo SPI de Java, se describe en el Anexo~D. Esta integración no reemplaza el flujo de anotación existente; añade un reconocedor dentro del mecanismo de extensión previsto por AVATecH \cite{auer2014avatech}. Una vez instalado, el reconocedor \textit{AI Orchestration Middleware} aparece en la lista de reconocedores de ELAN y permite seleccionar modelo, configurar el nivel de anotación destino y ejecutar inferencias sin manipular directamente los contenedores Docker.

\vspace{1\baselineskip}

\subsection{Estructura de directorios del sistema}

El Listado~\ref{lst:estructura_directorios} muestra cómo se organizó el middleware para reflejar la separación de responsabilidades definida en el diseño. La capa \textbf{api/} recibe solicitudes HTTP y serializa respuestas; \textbf{core/} centraliza configuración y logging; \textbf{schemas/} contiene los modelos Pydantic que hacen ejecutables los contratos; \textbf{services/} agrupa la lógica de instalación, registro, jobs, Docker, cola y métricas; \textbf{runners/} abstrae el mecanismo de ejecución; \textbf{storage/} conserva jobs en memoria durante la sesión; y \textbf{data/} mantiene información persistente mediante bind mounts.
...
\captionof{listing}{Estructura de clases Java integradas en ELAN}
...
\captionof{listing}{Comando PowerShell para comprimir un paquete de modelo}
