...
El diseño también estableció la estructura interna de clases del middleware, que se mantuvo estable durante todas las fases de implementación. La Figura~\ref{fig:diagrama_clases} presenta el diagrama de clases de los servicios principales con sus operaciones públicas y sus relaciones de uso. La capa de API recibe las solicitudes y las delega en dos coordinadores: \textbf{ModelRegistryService}, a cargo de la instalación y el registro de modelos, y \textbf{JobService}, a cargo de las inferencias. Ambos se apoyan en servicios especializados: la cola de trabajos, las métricas, la selección del mecanismo de ejecución y los servicios que hablan con Docker. La clase abstracta \textbf{BaseRunner} define el contrato de ejecución y \textbf{DockerRunner} lo implementa para backends en contenedores, de modo que otros mecanismos de ejecución podrían agregarse sin tocar el resto del sistema. Las clases Java integradas en ELAN se describen en el apartado de estructura de directorios y su inventario completo consta en el Anexo~D.

\begin{figure}[H]
\centering
\resizebox{0.97\textwidth}{!}{%
\begin{tikzpicture}[
  clase/.style={rectangle split, rectangle split parts=2, draw=black!75, fill=white,
                text width=3.55cm, align=left, font=\scriptsize, inner sep=3.5pt},
  capa/.style={rectangle, rounded corners=2pt, draw=black!75, fill=gray!14,
               text width=7.2cm, minimum height=0.9cm, align=center, font=\scriptsize},
  rel/.style={-{Stealth[length=2.2mm]}, semithick, black!75},
  her/.style={-{Triangle[open, length=3mm, width=2.4mm]}, semithick, black!75},
  eti/.style={font=\tiny, fill=white, inner sep=1pt}
]
% --- Capa API
\node[capa] (api) at (5.2,2.35) {\textbf{Capa API REST (routers FastAPI)}\\ /health \quad /api/v1/models \quad /api/v1/jobs \quad /api/v1/metrics};

% --- Fila 0: coordinadores
\node[clase] (regsvc) at (0,0)
  {\textbf{ModelRegistryService}
   \nodepart{second}
   +\,install\_model\_package()\\
   +\,list\_models()\\
   +\,get\_model()\\
   +\,update\_status()};
\node[clase] (jobsvc) at (5.2,0)
  {\textbf{JobService}
   \nodepart{second}
   +\,create\_segment\_video\_job()\\
   +\,get\_job()};
\node[clase] (queue) at (10.4,0)
  {\textbf{JobQueue}
   \nodepart{second}
   +\,submit()\\
   +\,queued\_count()\\
...
% --- Fila 1
\node[clase] (lifecycle) at (0,-3.0)
  {\textbf{DockerLifecycleService}
   \nodepart{second}
   +\,build\_and\_start()};
\node[clase] (selector) at (5.2,-3.0)
  {\textbf{RunnerSelector}
   \nodepart{second}
   +\,select()};
\node[clase] (metrics) at (10.4,-3.0)
...
% --- Fila 2
\node[clase] (docksvc) at (0,-6.0)
  {\textbf{DockerService}
   \nodepart{second}
   +\,ensure\_container()\\
   +\,wait\_for\_health()\\
   +\,post\_json()};
\node[clase] (runner) at (5.2,-6.0)
  {\textbf{DockerRunner}
   \nodepart{second}
   +\,run()};
\node[clase] (memstore) at (10.4,-6.0)
  {\textbf{MemoryStore}
   \nodepart{second}
   +\,save\_job()\\
   +\,get\_job()};

% --- Fila 3: clase abstracta
\node[clase] (baserunner) at (5.2,-8.3)
  {\textbf{BaseRunner} \{abstracta\}
   \nodepart{second}
   +\,run()};

% --- Relaciones
\draw[rel] (api.south -| regsvc.north) -- (regsvc.north);
\draw[rel] (api.south) -- (jobsvc.north);
\draw[rel] (jobsvc) -- (regsvc) node[eti, pos=0.5, above] {consulta modelos};
\draw[rel] (jobsvc) -- (queue) node[eti, pos=0.5, above] {encola};
\draw[rel] (jobsvc) -- (selector) node[eti, pos=0.5, right] {solicita runner};
\draw[rel] (jobsvc) -- (metrics) node[eti, pos=0.35, above, sloped] {registra};
\draw[rel] (jobsvc) -- (memstore) node[eti, pos=0.62, above, sloped] {guarda resultados};
\draw[rel] (selector) -- (runner) node[eti, pos=0.5, right] {instancia};
\draw[rel] (runner) -- (docksvc) node[eti, pos=0.5, above] {usa};
\draw[rel] (regsvc) -- (lifecycle) node[eti, pos=0.5, right] {instala};
\draw[rel] (lifecycle) -- (docksvc) node[eti, pos=0.5, right] {usa};
\draw[her] (runner) -- (baserunner) node[eti, pos=0.45, right] {hereda};
\end{tikzpicture}%
}
